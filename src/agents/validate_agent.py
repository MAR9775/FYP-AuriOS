...
def run_validation(software_name: str, software_type: str) -> str:
    ...
    print(f"Thought: Checking if '{software_name}' executable is accessible via PATH.")
    try:
        # 'which' command finds the full path of the executable in the system's PATH
        ...
        return "FAILURE: Validation failed. 'which' command not found (OS dependency issue)."
    ...
    if software_type.lower() == 'coding':
        ...
        test_script_path = os.path.join("/tmp", test_file_name)
        
        try:
            with open(test_script_path, "w") as f:
                f.write(f"print('Hello World! {software_name} validation successful.')\n")
            
            # Execute the test script using the located executable
        ...
        # Simple attempt to launch the application (platform-dependent)
        print("Thought: Attempting to launch the application for manual verification.")
        
        # Example for macOS/Linux (you'd need to adapt this command for Windows)
        try:
